        ...
        try {
          ...
          if (userFound) {
            const error = new Error(
              "Please enter a diffrent email, beacuse this is alredy exist"
            );
    ...
    body("confirmPassword")
      ...
      .custom((value, { req }) => {
        if (value !== req.body.password) {
          const error = new Error(
            "The password dont match, please enter again"
          );
          error.statusCode = 422;
          throw error;
        }
  ...
  [
    ...
    check("userId").custom(async (value, { req }) => {
      try {
        ...
        if (!userFound) {
          const error = new Error("This user dont exist anyomore");
